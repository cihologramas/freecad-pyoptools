...
class PowellLensGUI(WBCommandGUI):
    def __init__(self):
        ...
        WBCommandGUI.__init__(self,'PowellLens.ui')
...
class PowellLensMenu(WBCommandMenu):
    ...
    def GetResources(self):
        return {"MenuText": "Powell Lens",
                #"Accel": "Ctrl+M",
                "ToolTip": "Add Powell Lens",
                "Pixmap": ""}

class PowellLensPart(WBPart):
    def __init__(self,obj, R=3.00, CT=7.62, K=-4.302, D=8.89, matcat="", matref=""):
        WBPart.__init__(self,obj,"PowellLens")

        #Todo: Mirar como se puede usar un quantity
        obj.addProperty("App::PropertyPrecision","R","Shape","Curvature Radius Aspherical Surface ").R=(0,-10,10,1e-3)
        obj.addProperty("App::PropertyPrecision","K","Shape","Powell Lens Conicity ").K=(0,-10,10,1e-3)
        obj.addProperty("App::PropertyLength","CT","Shape","Powell Lens Center Thickness")
        obj.addProperty("App::PropertyLength","D","Shape","Powell Lens Diameter")
    ...
    def pyoptools_repr(self,obj):
        ...
        return comp_lib.PowellLens(radius=radius,thickness=thickness,K=K,R=curvature,material=material)
...
def buildlens(R,CT,K,D):
    ...
    e.translate(FreeCAD.Base.Vector(-D/2.,0,0))
    
    d=Part.makeCylinder(D/2.,CT)
